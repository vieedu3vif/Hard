...
{
   __implicit = 0;
   lora_write_reg(REG_MODEM_LORA_1, lora_read_reg(REG_MODEM_LORA_1) & 0xfe);
}
...
{
   __implicit = 1;
   lora_write_reg(REG_MODEM_LORA_1, lora_read_reg(REG_MODEM_LORA_1) | 0x01);
...
lora_set_spreading_factor(int sf)
{
   ...
   lora_write_reg(REG_MODEM_LORA_2, (lora_read_reg(REG_MODEM_LORA_2) & 0x0f) | ((sf << 4) & 0xf0));
}
...
lora_set_bandwidth(long sbw)
{
   ...
   lora_write_reg(REG_MODEM_LORA_1, (lora_read_reg(REG_MODEM_LORA_1) & 0x0f) | (bw << 4));
}
...
lora_set_coding_rate(int denominator)
{
   ...
   lora_write_reg(REG_MODEM_LORA_1, (lora_read_reg(REG_MODEM_LORA_1) & 0xf1) | (cr << 1));
}
...
{
   lora_write_reg(REG_MODEM_LORA_2, lora_read_reg(REG_MODEM_LORA_2) | 0x04);
}
...
{
   lora_write_reg(REG_MODEM_LORA_2, lora_read_reg(REG_MODEM_LORA_2) & 0xfb);
}
...
//   gpio_set_direction(LORA_RST_GPIO, GPIO_MODE_OUTPUT);
   gpio_set_direction(LORA_RST_GPIO, GPIO_MODE_OUTPUT);
//   gpio_pad_select_gpio(LORA_CS_GPIO);
   ...
   lora_write_reg(REG_MODEM_LORA_3, 0x04);
